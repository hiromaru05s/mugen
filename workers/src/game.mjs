// 夢幻の森 — ゲーム本体(サーバー実装から独立)
// Roomシム(接続・ブロードキャスト・タイマー)を注入して使う:
//   Cloudflare Workers版 → src/room.mjs (Durable Object)
//   Node版(VM)          → gameserver/node-room.mjs (ws)
// ゲームロジックは1つだけ。移設してもルールが分岐しないようにするための構成。
'use strict';
import { rankOf } from './shared.mjs';
import cfgFile from '../config.json' with { type: 'json' };
...
/** RoomBase(接続シム)を受け取ってForestRoomクラスを生成する */
export function defineForestRoom(Room) {
return class ForestRoom extends Room {
  matchLeft() { return Math.max(0, Math.round(CFG.matchLen - this.t)); }
  onCreate() {
    ...
    this.onMessage('ready', (client) => {
      ...
      client.send('init', {
        ...
        chests: this.world.chests.map(c => ({ x: c.x, y: c.y })),
  ...
  tick(dt) {
    ...
    if (this.dragon && this.dragon.hp <= 0) this.endMatch(true, 'dragon');
    else if (units.length > 0 && alive.length === 0) this.endMatch(false, 'wipe');
    else if (this.t >= CFG.matchLen) this.endMatch(false, 'timeout');
  ...
  endMatch(dragonKilled, reasonCode) {
    if (this.over) return;
    // 表示文字列はクライアントがreasonCodeから翻訳する(i18n)。reasonはログ/旧クライアント用
    const reason = ({ dragon: '竜は倒れた', wipe: '全滅 — 森が勝った', timeout: '時間切れ — 森が全てを呑んだ' })[reasonCode] || reasonCode;
    ...
    this._registryFetch('POST', '/record',
      ...
      .then(players => {
        this.result = { dragonKilled, reason, reasonCode, rows, players };
  ...
  broadcastSnapshots() {
    ...
    // 座標はinitで送信済み — 毎スナップは解錠フラグの配列だけ(0/1)
    const gimsPub = this.world.gimmicks.map(g => g.unlocked ? 1 : 0);
    const chestsPub = this.world.chests.map(c => c.open ? 1 : 0);
    for (const client of this.clients) {
      ...
      const teamUnits = units
        ...
        .map(u => ({
          // 帯域最適化: false/null/0のフラグはundefinedにしてJSONから消す(クライアントは真偽値として読むので等価)
          id: u.id, team: u.team, cls: u.cls, skin: u.skin || undefined, x: Math.round(u.x), y: Math.round(u.y),
          hp: Math.round(u.hp), maxHp: this.maxHpOf(u), facing: +u.facing.toFixed(2), dead: u.dead || undefined,
          reveal: u.revealT > 0 || undefined, stealth: (u.team === team && u.stealth > 0) || undefined,
          stun: u.stunT > 0 || undefined, shield: u.shieldT > 0 || undefined, mark: u.markT > 0 || undefined, lv: u.lv,
          name: u.name, bot: u.bot || undefined,
          downed: u.downed || undefined, downT: u.downed ? +Math.max(0, u.downT).toFixed(1) : undefined,
        }));
      ...
      client.send('snap', {
        ...
        mobs: this.world.mobs.filter(m => m.hp > 0 && near(m))
          .map(m => ({ id: m.id, x: Math.round(m.x), y: Math.round(m.y), hp: Math.round(m.hp), maxHp: Math.round(m.maxHp), tele: m.tele ? { x: m.tele.x, y: m.tele.y, r: m.tele.r } : undefined })),
        zones: this.zones.filter(z => near(z)).map(z => ({ x: z.x, y: z.y, r: z.r, kind: z.kind, tele: +Math.max(0, z.tele).toFixed(2), team: z.team })),
        walls: this.walls.filter(w => near(w)).map(w => ({ x: Math.round(w.x), y: Math.round(w.y), r: w.r })),
        projs: this.projs.filter(p => near(p)).map(p => ({ x: Math.round(p.x), y: Math.round(p.y), r: p.r, cls: p.src.cls })),
        ...
        chests: chestsPub, gimmicks: gimsPub, dragon: dragonPub, boss: bossPub, npc: npcPub, shares,
        trails: this.trails.filter(tr => near(tr)).map(tr => ({ x: Math.round(tr.x), y: Math.round(tr.y), own: tr.team === team || undefined, f: +Math.min(1, tr.life / CFG.trailLife).toFixed(2) })),
        ...
        me: {
          ...
          nearMerchant: this.world.merchants.some(m => dist(me, m) < 70) || undefined,
          nearGim: this.world.gimmicks.some(g => !g.unlocked && dist(me, g) < 60) || undefined,
          gimActive: !!me.gim || undefined,
          nearDowned: [...this.units.values()].some(a => a.team === me.team && a.downed && !a.dead && dist(a, me) < 50) || undefined,
          revP: me.revCh ? +Math.min(1, (this.t - me.revCh.t0) / CFG.reviveTime).toFixed(2) : undefined,
          buildOffer: buildOffer || undefined, builds: [me.b1, me.b2],
        },
      });
    }
  }
