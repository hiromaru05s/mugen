// 夢幻の森 M2 サーバー — マッチループ完成版 (SPEC §5/§6 M2)
// M1(スキルプリミティブ)に追加: バンドマップ / 成長(xp・lv・gold) / 宝箱 / 商人 / 封印装置(サーバー検証QTE) /
// 枯死圧(DoT+視界悪化) / ドラゴンv0(フェーズ+予兆AoE) / チームRP精算 / 試合フロー(死=脱落、観戦)
'use strict';
...
class ForestRoom extends Room {
  onCreate() {
    ...
    console.log('[forest:m2] room created', this.roomId, DEV ? '(DEV)' : '');
  }
  ...
  onJoin(client, options) {
    ...
    console.log('[forest:m2] join', client.sessionId, 'team', team, cls);
  }
  ...
  endMatch(dragonKilled, reason) {
    ...
    this.result = {
      dragonKilled, reason,
      rows: [0, 1].map(team => ({ team, dmg: Math.round(this.dragonDmg[team]), share: +(this.dragonDmg[team] / total).toFixed(3), rp: Math.round(this.dragonDmg[team] / total * 1000) })),
    };
  ...
  broadcastSnapshots() {
    ...
    for (const client of this.clients) {
      ...
      if (!me) continue;
      client.send('snap', {
        t: +this.t.toFixed(3), witherR: Math.round(this.witherR), over: this.over,
...
gameServer.listen(port).then(() => console.log(`[forest:m2] listening on ws://localhost:${port}`));
